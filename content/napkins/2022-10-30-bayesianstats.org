...
In Logic we have operators for reasoning, these are \( \in \lbrace AND, OR, NOT \rbrace \) and their descriptive natural language counterparts: ~Conjunction, Disjunction, Negation~ -- which when combined with /True/ data we can reason about the world.
...
- \( NOT \mapsto \neg P(A) = 1 - P(A) ; \) -- (the “@@html:<strong>¬</strong>@@” symbol refers to “the negation of”, in this case the negation of the Probability of A is equal to 1-P(A), effectively the probability of *not* A)
