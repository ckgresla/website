...
1. [[https://en.wikipedia.org/wiki/Exploratory_factor_analysis][Exploratory Factor Analysis]] (EFA): make no /a priori/ assumptions about what Variables will decompose into what factors (let the data tell you what combinations of Variables will make good Factors)
   reduce the number of input features
2. [[https://en.wikipedia.org/wiki/Confirmatory_factor_analysis][Confirmatory Factor Analysis]] (CFA): looks to test a hypothesis on Actual Variables (assuming that some combination of Variables X & B decompose nicely into Factor x̄)
