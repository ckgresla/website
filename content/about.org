#+title: About
#+tagline: Why hello there!
#+body_class: center
#+date: 2018-11-28T15:15:34+10:00
#+base_image: /assets/images/personal/profile-250px.jpg
#+focus_image: /assets/images/personal/squirtle-tongue.gif

Im Chris Kerwell Gresla, an Indian & German, American bloke from Chicago. I love statistics, food, music (like jazz), life, and cool people/models (probably like you if you are reading this).

Currently I am doing a fair bit of machine learning at [[https://wafer.systems/][Wafer Systems]]. We are making an “operating system that understands you better than yourself” -- my work spans the OS's model surface: the device-use agents (SFT through RLVR), the search engine over everything your phone knows, and the models and systems that personalize the OS from what it ingests of your life. Some public bits of that work: [[https://wafer.systems/blog/edm/][Evolutionary Data Making]], where frontier models act as evolutionary operators to grow our training data, and [[https://github.com/ckgresla/representax][representax]], an open-source JAX library for representation learning. There is still more context than fits this section (lets chat).

At the moment, I am based in San Francisco.


If you care, here are some places I reside online: [[https://github.com/ckgresla][ 
  [Github]], [[https://x.com/thepapabosh][X]], [[https://www.linkedin.com/in/chriskerwellgresla/][LinkedIn]], [[https://open.spotify.com/user/1258747599?nd=1][Spotify]]
  ]



#+begin_export html
<div style="margin-top: 2rem; margin-bottom: 20px; text-align: left;">
  <figure style="margin: 0; max-width: 500px;">
    <img src="/assets/images/misc/tiny-heater.png" alt="tiny-heater" style="width: 100%;">
    <figcaption>A tiny "heater" in the studio</figcaption>
  </figure>
</div>
#+end_export

#+begin_export html
<div style="text-align: right;">
  <div style="display: inline-block; max-width: 500px;">
    {{< image-swap
      link="https://www.youtube.com/watch?v=Uj07-YU5cTk"
      full="true"
      focus_src="/assets/images/misc/focused-desk-flower.png"
      base_src="/assets/images/misc/focused-desk-hhkb.png"
      alt="Image Description"
      title="A wild HHKB and Silent Princess inhabit a desk"
    >}}
  </div>
</div>
#+end_export


#+begin_export html
<br>
#+end_export

#+begin_export html
<br>
#+end_export


As one should not think twice about an opportunity, one shouldn’t hesitate to write down a good quote when you find it -- here is one I had the pleasure of coming about recently:

#+begin_quote
“Style’s the hardest thing to get, and it’s not something you really strive for. I’m sure that Miles never really strived to be a stylist. He just strived to be himself, to learn, to develop, and to express a strong, independent personality. And at the end of it, it had an identity and that’s why he’s a stylist. But somebody might say, ’Wow, I could be a stylist by just reaching out and being strange.’ or reaching out and being different, or novel, and of course that’s a mistake.”
#+end_quote

     [[https://www.youtube.com/watch?v=w2Zl9hm9VFE&t=1544s&ab_channel=%EC%9E%AC%EC%A6%88%EA%B8%B0%EC%9E%90JazzEditor][Bill Evans]]


#+begin_export html
<br>
#+end_export

#+begin_export html
<br>
#+end_export


If you made it this far, you should probably say hello!

ckgresla at gmail dot com

~¯\_(ツ)_/¯~
